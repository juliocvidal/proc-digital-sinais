...
% Resposta janela hamming

%###########################IMPORTANTE##############################
%quando eu chegar do trabalho eu vejo como ficou o restante com vcs
%para modificar as janelas, basta mudar o parametro de hamming para
%rectwin, por exemplo
BM = fir1((N-1),Wn,'stop',hamming(N));
%Calculos para Resposta em Freqüência
%Hamming
[Hr,wr] = freqz(BM,1);
%Gráficos modulo/fase para janela hamming
figure(1);
subplot(2,1,1);
%Gráfico de modulo Db x Hz
plot(wr*Ws/(2*pi),20*log10(abs(Hr)));
grid minor;
title('Filtro Rejeita-faixa Janela Hamming - Resposta em Frequência');
xlabel('Hertz');
ylabel('(dB)'); 
...
% MultiSenoidal Filtrado: janela hamming
