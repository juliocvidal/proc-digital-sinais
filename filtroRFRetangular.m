...
%para modificar as janelas, basta mudar o parametro de retangular para
%rectwin, por exemplo
BM = fir1((N-1),Wn,'stop',rectwin(N));
...
grid minor;
title('Filtro Rejeita-faixa Janela Retangular - Resposta em Frequência');
xlabel('Hertz');
ylabel('(dB)'); 
...
% MultiSenoidal Filtrado: janela retangular
...
MTFSMFR = abs(fft(MFBM)); %Janela Retangular          
f = Ws*(0:399)/800; 
figure(3);
subplot(2,1,1);
plot(f,MTFSM(1:400)); 
