...
% Resposta janela bartlett

%###########################IMPORTANTE##############################
%quando eu chegar do trabalho eu vejo como ficou o restante com vcs
%para modificar as janelas, basta mudar o parametro de bartlett para
%rectwin, por exemplo
BM = fir1((N-1),Wn,'stop',bartlett(N));
%Calculos para Resposta em Freqüência
%Bartlett
[Hr,wr] = freqz(BM,1);
%Gráficos modulo/fase para janela bartlett
figure(1);
subplot(2,1,1);
%Gráfico de modulo Db x Hz
plot(wr*Ws/(2*pi),20*log10(abs(Hr)));
grid minor;
title('Filtro Rejeita-faixa Janela Bartlett - Resposta em Frequência');
xlabel('Hertz');
ylabel('(dB)'); 
...
% MultiSenoidal Filtrado: janela bartlett
